...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	cloudnine: {
		onStart(source) {
			...
			if (this.field.isWeather(['snowscape', 'hail'])) {
				this.boost({ def: 1 });
	...
	quickdraw: {
		...
		onFractionalPriority(priority, pokemon, target, move) {
			if (pokemon.volatiles['quickdraw']?.turns >= 3 && pokemon.item !== 'galaricacuff') {
				...
				return 0.1;
			} else if (pokemon.volatiles['quickdraw']?.turns >= 2 && pokemon.item === 'galaricacuff') {
				pokemon.volatiles['quickdraw'].turns = 0;
				this.add('-activate', pokemon, 'ability: Quick Draw');
				return 0.1;
			}
	...
	sandforce: {
		...
		onBasePower(basePower, attacker, defender, move) {
			...
			} else if (move.type === 'Rock' || move.type === 'Ground' || move.type === 'Steel') {
				this.debug('Sand Force boost');
				return this.chainModify(1.25);
			}
		},
	...
	slushrush: {
		onModifySpe(spe, pokemon) {
			if (this.field.isWeather(['hail', 'snowscape']) || pokemon.volatiles['moonball']) {
				return this.chainModify(2);
			}
		},
		onEnd(pokemon) {
	...
	stoneknuckles: {
		...
		onBasePower(basePower, attacker, defender, move) {
			if (move.flags['punch']) {
				this.debug('Stone Knuckles boost');
				return this.chainModify(2);
			}
		},
		onAfterMoveSecondarySelf(source, target, move) {
	...
	assertiveintelligence: {
		onStart(pokemon) {
			this.add('-ability', pokemon, 'Assertive Intelligence');
		},
		onModifyMove(move) {
			move.ignoreAbility = true;
	...
	wattfeather: {
		onStart(pokemon) {
			pokemon.addVolatile('wattfeather');
		},
		onResidualOrder: 1,
		onResidualSubOrder: 1,
		onResidual(pokemon) {
			if (pokemon.volatiles['wattfeather']) {
				pokemon.volatiles['wattfeather'].turns++;
			}
		},
		onBasePowerPriority: 19,
		onBasePower(basePower, attacker, defender, move) {
			if (attacker.volatiles['wattfeather']?.turns >= 3 || attacker.status === 'par') {
				this.debug('Watt Feather boost');
				attacker.volatiles['wattfeather'].turns = 0;
				this.add('-activate', attacker, 'ability: Watt Feather');
				return this.chainModify(2);
			}
		},
		condition: {
	...
	envelopingdarkness: {
		onTryHit(pokemon, target, move) {
			if (move.flags['light']) {
				this.add('-immune', pokemon, '[from] ability: Enveloping Darkness');
				return null;
			}
		},
		onAnyAccuracy(accuracy, target, source, move) {
	...
	sacredletters: {
		onPrepareHit(source, target, move) {
			...
			const type = move.type;
			if (type && type !== '???' && source.getTypes().join() !== type &&
				(move.id === 'hiddenpower' || move.placeholderFor === "Hidden Power")) {
				if (!source.setType(type)) return;
				this.effectState.protean = true;
				this.add('-start', source, 'typechange', type, '[from] ability: Sacred Letters');
			}
		},
		onSwitchIn(pokemon) {
		...
		onTryMove(source, target, move) {
			...
			if ((target.hasType('Normal') || target.hasType('Rock') || target.hasType('Ice') || target.hasType('Steel') ||
				...
				type.push("Fighting");
			} if ((target.hasType('Bug') || target.hasType('Grass') || target.hasType('Ice') || target.hasType('Steel')) &&
				!(target.hasType('Rock') || target.hasType('Fire') || target.hasType('Water') || target.hasType('Dragon'))) {
				type.push("Fire");
			} if ((target.hasType('Ground') || target.hasType('Rock') || target.hasType('Fire')) &&
				!(target.hasType('Water') || target.hasType('Grass') || target.hasType('Dragon'))) {
				type.push("Water");
			} if ((target.hasType('Fighting') || target.hasType('Bug') || target.hasType('Grass')) &&
				!(target.hasType('Rock') || target.hasType('Electric') || target.hasType('Steel'))) {
				type.push("Flying");
			} if ((target.hasType('Ground') || target.hasType('Rock') || target.hasType('Water')) &&
				...
				type.push("Bug");
			} if ((target.hasType('Dragon')) && !(target.hasType('Steel') || target.hasType('Fairy'))) {
				type.push("Dragon");
			} if ((target.hasType('Ghost') || target.hasType('Psychic')) && !(target.hasType('Dark') ||
				target.hasType('Normal'))) {
				type.push("Ghost");
			} if ((target.hasType('Ghost') || target.hasType('Psychic')) && !(target.hasType('Dark') ||
				target.hasType('Fighting') || target.hasType('Fairy'))) {
				type.push("Dark");
			} if ((target.hasType('Rock') || target.hasType('Ice') || target.hasType('Fairy')) &&
				!(target.hasType('Steel') || target.hasType('Fire') || target.hasType('Water') || target.hasType('Electric'))) {
				type.push("Steel");
			} if ((target.hasType('Fighting') || target.hasType('Dragon') || target.hasType('Dark')) &&
				!(target.hasType('Poison') || target.hasType('Steel') || target.hasType('Fire'))) {
				type.push("Fairy");
			}
			if (move.id === 'hiddenpower' || move.placeholderFor === "Hidden Power") {
				rand = (type.length - 1);
				const r = this.random(rand);
				move.type = type[r];
			}
		},
		flags: { },
	...
	gigantificationschooling: {
		onSwitchInPriority: -1,
		onStart(pokemon) {
			if (pokemon.baseSpecies.baseSpecies !== 'Wishiwashi-Totem' || pokemon.level < 20 || pokemon.transformed) return;
			if (pokemon.hp > pokemon.maxhp / 4) {
		...
		onResidual(pokemon) {
			if (
				pokemon.baseSpecies.baseSpecies !== 'Wishiwashi-Totem' || pokemon.level < 20 ||
				pokemon.transformed || !pokemon.hp
			) return;
			if (pokemon.hp > pokemon.maxhp / 4) {
				if (pokemon.species.id === 'wishiwashitotem') {
